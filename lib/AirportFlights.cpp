...
vector<AirportFlights> AirportFlights::getDatabase()
{
    ...
    if(!is_directory(parent_path)) {
        cout << "Error: database does not exist, please run the script to create the database." << endl;
        throw exception();
    }
    ...
    for (const auto& entry : directory_iterator(parent_path)) {
        if (entry.is_directory()) {
            ...
            if(!file_arrival.is_open()) {
                cout << "Error: getting the database, please run the script to create the database." << endl;
                throw exception();
            }
